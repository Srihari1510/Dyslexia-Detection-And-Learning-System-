...
# Generate quiz and paragraph
def generate_quiz(risk_score):
    try:
        prompt = f"""
        Generate a 10-question multiple-choice quiz to test reading and comprehension skills, tailored to a Dyslexia Risk Score of {risk_score}/100. 
        The difficulty should increase with higher risk scores (e.g., simpler questions for low scores, more challenging for high scores). 
        Each question must:
        - Have exactly 2 unique answer options (labeled 'a' and 'b').
        - Be unique in content and wording, with no repetition of question themes or structures across the quiz.
        - Focus on reading, comprehension, or dyslexia-related skills (e.g., letter recognition, word differentiation, or sentence understanding).
        Additionally, generate a 5-line paragraph (5 sentences) about any topic of your choice, suitable for display on a web page.
        Return the response in JSON format with the structure:
        {{
            "quiz": [
                {{"question": "text", "options": {{"a": "text", "b": "text"}}, "correct": "letter"}},
                ...
            ],
            "paragraph": "text"
        }}
        Ensure the response is valid JSON, the quiz contains exactly 10 questions, and the paragraph has exactly 5 sentences.
        """
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "You are an educational assistant creating quizzes and content for dyslexia assessment."},
        ...
        if not quiz_json.startswith('{'):
            quiz_json = quiz_json[quiz_json.find('{'):quiz_json.rfind('}') + 1]
        result = json.loads(quiz_json)
        if not isinstance(result, dict) or 'quiz' not in result or 'paragraph' not in result:
            raise ValueError("Invalid response format")
        quiz = result['quiz']
        paragraph = result['paragraph']
        if not isinstance(quiz, list) or len(quiz) != 10 or not all(isinstance(q, dict) and 'options' in q and len(q['options']) == 2 for q in quiz):
            raise ValueError("Invalid quiz format or incorrect number of questions/options")
        if len(paragraph.split('. ')) < 5:  # Basic check for 5 sentences
            raise ValueError("Paragraph does not contain 5 sentences")
        return {'quiz': quiz, 'paragraph': paragraph}
    except Exception as e:
        print(f"Error generating quiz or paragraph: {str(e)}. Using default.")
        return {
            'quiz': [
                {
            ...
            'paragraph': (
                "The sky is vast and full of wonder. Stars twinkle like distant dreams. Clouds drift slowly, carrying stories of faraway places. "
                "The moon glows softly, guiding night travelers. Each sunrise brings new hope."
            )
        }
...
def quiz():
    ...
    if request.method == 'POST':
        if 'risk_score' in request.form and 'quiz_answers' not in request.form:
            try:
                ...
                quiz_data = generate_quiz(risk_score)
                quiz = quiz_data['quiz']
            ...
            try:
                ...
                if not quiz_data_raw:
                    return render_template('quiz.html', error="Quiz data is missing", paragraph=paragraph)
                ...
                if len(quiz_data) != len(user_answers):
                    return render_template('quiz.html', error="Mismatch between quiz questions and answers", paragraph=paragraph)
    ...
    return render_template('quiz.html', quiz=quiz, quiz_score=quiz_score, risk_score=risk_score, paragraph=paragraph, error=error)
